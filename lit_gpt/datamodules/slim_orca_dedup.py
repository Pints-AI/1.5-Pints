...
DOWNLOAD_DIR = './data/sft/slimorca-dedup'
SPLIT = ['train']

class PreparedSlimOrcaDedup(TypedDict):
...
OpenOrcaConversationTurn = TypedDict(
    'OpenOrcaConversation', {
        'from': Literal['system', 'human', 'gpt'], 'value': str}
)


class SlimOrcaDedupRow(TypedDict):
    conversations: List[OpenOrcaConversationTurn]


@dataclass
class SlimOrcaDedup(DataModule):
    ...
    def setup(self, stage: str = '') -> PreparedSlimOrcaDedup:
        dataset = self.prepare_data()
        train_data = format_dataset(
            dataset[0], self.include_multiturn_conversations)
        # TODO: SlimOrcaDedup doesn't have test_data. We can split it out from the train_data
...
def format_dataset(
    ...
    for entry in dataset:
        ...
        if human_message['from'] != 'human':
            print(
                f'WARN: A SlimOrca row is corrupted. Expected role to be `user`, but is `{human_message["from"]}` instead.'
            )

        ai_message = conversation[2]
        if ai_message['from'] != 'gpt':
            print(
                f'WARN: A SlimOrca row  is corrupted. Expected role to be `assistant`, but is `{ai_message["from"]}` instead.'
            )
