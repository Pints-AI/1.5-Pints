...
try:
    ...
    print(e)
    match input("No xformers. It won't work. Continue for debugging? (yes/no): "):
        case 'yes':
            pass
        case _:
            exit()
try:
    ...
    print(e)
    match input(
        "No fused rotary embedding. It won't work. Continue for debugging? (yes/no): "
    ):
        case 'yes':
            pass
        case _:
...
class GPT(nn.Module):
    ...
    def max_seq_length(self, value: int) -> None:
        ...
        if not hasattr(self, "cos"):
            # first call
            cos, sin = self.get_rope_cache()
            self.register_buffer("cos", cos, persistent=False)
            self.register_buffer("sin", sin, persistent=False)
        # override
        elif value != self.cos.size(0):
            self.cos, self.sin = self.get_rope_cache(device=self.cos.device)
    ...
    def get_rope_cache(self, device: Optional[torch.device] = None) -> Tuple[torch.Tensor, torch.Tensor]:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        cos, sin = rope
        # Fixes RuntimeError: Expected x1.dtype() == cos.dtype() to be true, but got false.
        # See [TODO: Add the reference]
        cos, sin = cos.clone(), sin.clone()
