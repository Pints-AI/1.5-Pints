...
def choose_logger(
    ...
    **kwargs: Any,
    ...
    if logger_name == 'wandb':
        return WandbLogger(project=project_name, resume=resume, **kwargs)
    raise ValueError(
        f"`--logger_name={logger_name}` is not a valid option. Choose from 'csv', 'tensorboard', 'wandb'."
    )


def humanize_seconds(seconds: Union[float, int]):
    days = seconds // (24 * 3600)
    seconds %= 24 * 3600
    hours = seconds // 3600
    seconds %= 3600
    minutes = seconds // 60
    return f'{days} day(s), {hours} hr(s), {minutes} min(s)'


def remove_model_prefix(state_dict: Dict[str, torch.Tensor]) -> None:
    """
    Handles the case where the LightningModule wrapper (FullPintsTransformer) prepends 'model.'
    to all keys in the state dict. Removes the prefix in-place.
    """
...
def add_model_prefix(state_dict: Dict[str, torch.Tensor]) -> None:
    """
...
def load_weights(
    ...
    """
    assert not (remove_prefix and add_prefix), \
        'Cannot add and remove model prefix to the state dict simultaneously.'

    state = torch.load(path)

    # Handle cases where state_dict has 'model' key
    state = state.get('model', state)

    if remove_prefix:
        remove_model_prefix(state)

    if add_prefix:
        add_model_prefix(state)
